...
#error "Please define a frequency band in the compiler options."
   ...
   // flag_autosend=1;
    ...
    while( 1 )
    {
        ...
        case RX_ERROR:
            WaitRX=0;
            UartPrint("v1 RX_TIMEOUT/RX_ERROR\n");
                State = LOWPOWER;
            break;
        case TX_TIMEOUT:
            State = LOWPOWER;
...
void OnRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
{
    ...
    RssiValue = rssi;
	  //snr=snr[twos complement]
	  //SnrValue = snr;
	  if(snr &0x80)// The SNR sign bit is 1  
		{
		// Invert and divide by 4
		SnrValue= ((~snr+1)&0xFF) >> 2;
		SnrValue = -SnrValue;			
		}
		else
		{
...
void mtype_check(void)
{
    ...
    b=handle<<(0);
    c=b>>(7);
	//(((handle & 0xFF) << 0)>>7) is OK
    if(c) //1
    {
        b=handle<<(1);
        c=b>>(7);
			//(((handle & 0xFF) << 1)>>7) is OK
        if(c)//11
        {
            b=handle<<(2);
            c=b>>(7);
