...
//0421 ADR testing
    ...
    else if(strncmp( ( const char*) uart_cmd, ( const char*) "setDevAddr", 10) == 0)
    {
        ...
        setDevAddr(dDvAddr);
		 ...
		 else if(strncmp( ( const char*) uart_cmd, ( const char*) "classbtimer", 11) == 0)
    {
        uint32_t time;
        uint8_t a[12];
        UartPrint("command:classbtimer\n");
...
/*
    ...
    else if(strncmp( ( const char*) uart_cmd, ( const char*) "LinkADRReq", 10) == 0)
    {
        ...
        LinkADRReq(_state, _datarate, _txpower, 0, NbTrans, 0);
    }
        ...
        if((datarate != 7) || (txpower != 2))
        {
            if(autosendCount == 29)
            {
                ...
                temp_txpower=txpower;
									LinkADRReq(0, datarate, 14, 0, NbTrans, 0);
